...
def get_corresponding_log_name(instrument_logs, t0_image):
    for t0, tf, log_name in instrument_logs:
        if t0 <= t0_image and t0_image <= tf:
            print("log_found! Using log: ", log_name)
            return log_name
...
def read_logs(log_name, path_data, path_logs):
    try: 
        ...
        if df_log_temperature.empty:
            print('     Pressure file is empty')
        ...
        if df_log_pressure.empty:
            print('     Pressure file is empty')
            df_log_pressure = None

        # Read log resistance if exists
        file_resistance_path = path_data + path_logs + "resistance/resistance_" + log_name + ".csv"
        if os.path.exists(file_resistance_path):
            df_log_resistance = pd.read_csv(file_resistance_path, sep=',')
            df_log_resistance["Time_relative"] = df_log_resistance["Time"] - t0
            if df_log_resistance.empty:
                print('     Resistance file is empty')
        ...
        print('     Could not read the pressure, temperature logs')
...
def create_log(sample_name, path_data, path_logs, path_raw, path_save, instrument_logs):
    ...
    for scan_id in scans_id_set:
        ...
        try:
            ...
            df_log_temperature, df_log_pressure, df_log_resistance = read_logs(log_name, path_data, path_logs)
...
def main():
    ...
    path_data = str(input("Enter the path where the data is located in your computer: "))
    print("-------------------------------")
    path_raw = "/RAW/"
    path_logs = "/DEVICES_RAW/"
    path_save = path_data + "/PROCESSED/logs/"

    # sample = "Y12209"
    # path_data = "E:/WD Discovery/ALBA March 2024/DATA"
    # path_raw = "/RAW/"
    # path_logs = "/DEVICES_RAW/"
    # path_save = "C:/Users/omola/Desktop/ALBA analysis/ALBA March24/logs/"
