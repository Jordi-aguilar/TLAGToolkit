...
class data_analysis_NCD:
    def __init__(self, thickness, process, REBCO_name, time_before_jump, time_after_jump, save_figure, save_file, directory_data, time1_average_grate, time2_average_grate):
        ...
        self.colors_peaks = {
            ...
            f'{self.REBCO_name}(103)(110)': 'olive',
            'Y2Cu2O5': 'gold',
    ...
    def find_time_jump(self):
        pressure_derivative = calculate_derivative(self.experiment_data['time'], self.experiment_data['pressure'])
    ...
    def average_growth_rate(self, measurement, time_line1, time_line2):
        ...
        index_YBCO = self.find_index_peak(self.peaks, f'{self.REBCO_name}(005)')
        index_jump = self.find_index_of_closest_num(self.peaks[index_YBCO]['time'], self.time_jump)
        index_cooling = self.find_index_of_closest_num(self.peaks[index_YBCO]['time'], self.time_cooling)
        
        x_axis_plot = self.peaks[index_YBCO]['time'][index_jump-10:index_cooling-150]
        y_axis_plot = self.peaks[index_YBCO][label][index_jump-10:index_cooling-150]
    ...
    def start_analysis(self):
        ...
        for key in ['I_max', 'AUC', 'I_max_norm', 'AUC_norm']:
        ...
        if self.process == 'heating':
            index_end_heating = find_start_of_decrease(self.experiment_data['temperature'][self.experiment_data['index_start']:])
...
def main():
    ...
    directory_data = 'E:/ALBA March 24/POSTPROCESSED/'  # you should change it for the directory where your data is
    thickness = 400            # in nm
    process = 'jump'        # heating, jump, cooling
    REBCO_name = 'YBCO'        # YBCO, GdBCO,YGdBCO... Whatever name you gave in the excels
    time_before_jump = 10      # for the plot at jump (in s)
    time_after_jump = 80      # for the plot at jump (in s). If set to 'cooling' it takes the time that cooling begins
    ...
    time_line2_average_grate = 50   # (in s) seconds straight line goes from (time_line2_average_grate, cooling)

    XRD_analysis = data_analysis_NCD(thickness, process, REBCO_name, time_before_jump, time_after_jump, save_figure, save_file, directory_data, time_line1_average_grate, time_line2_average_grate)
